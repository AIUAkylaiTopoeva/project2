...
class UpdateMixin(GetMixin):
    def update(self):
        ...
        while True:
            choice = input('Что вы хотите изменить? 1 - brand, 2 - model, 3 - year, 4 - engine, 5 - color, 6 - kuzov, 7 - probeg, 8 - price : ')
...
class Cars(CreateMixin,ListingMixin,RetrieveMixin,UpdateMixin,DeleteMixin):
    def __init__(self,brand = '',model = '',year = 0,engine = 0,color = '',kuzov = '',probeg = 0,price = 0):
        self.brand = brand
        self.model = model
        self.year = year
        self.engine = engine
